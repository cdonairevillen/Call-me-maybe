...
class LLMEngine:
    ...
    BOOL_TYPES: frozenset[str] = frozenset({"bool", "boolean"})

    BOOL_TRUE_WORDS: frozenset[str] = frozenset({
        "true", "yes", "on", "enable", "enabled",
        "si", "forward", "ascending",
    })

    BOOL_FALSE_WORDS: frozenset[str] = frozenset({
        "false", "no", "off", "disable", "disabled",
        "descending", "reverse", "backwards",
    })
    ...
    def is_bool(self, type_str: str) -> bool:
        """Return True if type_str represents a boolean parameter type."""
        return type_str.lower() in self.BOOL_TYPES
    ...
    def build_selection_prompt(self, prompt: str,
        ...
        return (
            ...
            " first. A greeting word at the start does NOT override this.\n"
            ...
            " hello/hi at the START of a question is NOT greet.\n"
    ...
    def extract_bool_param(self, prompt: str, param_name: str) -> bool:
        """Extract a boolean value from a prompt.

        Searches for the parameter name followed by a true/false word,
        then falls back to scanning the whole prompt for any boolean
        signal. Returns False when no signal is found.
        """
        low = prompt.lower()

        scoped = re.search(
            r"\b" + re.escape(param_name.lower()) + r"\b.{0,20}",
            low,
        )
        scope = scoped.group(0) if scoped else low

        for word in self.BOOL_TRUE_WORDS:
            if re.search(r"\b" + re.escape(word) + r"\b", scope):
                return True
        for word in self.BOOL_FALSE_WORDS:
            if re.search(r"\b" + re.escape(word) + r"\b", scope):
                return False
    ...
    def build_candidates(self, prompt: str, param_type: str,
        ...
        if self.is_bool(param_type):
            return [self.extract_bool_param(prompt, param_name)]
    ...
    def cast_value(self, value: Any, param_type: str,
        ...
        if self.is_bool(param_type):
            if isinstance(value, bool):
                return value
            return str(value).lower() in self.BOOL_TRUE_WORDS
    ...
    def resolve_string_roles(
        self, string_params: list[str],
    ) -> dict[str, str]:
        """Map each string parameter name to a canonical extraction role.

        Three levels of resolution, applied in order:

        1. Exact match — parameter is named exactly 'source_string',
           'regex', or 'replacement'. Uses its own dedicated extractor.
        2. Partial match — at least one parameter matches exactly.
           Unrecognised parameters fill the remaining positional slots
           in the order: source_string → regex → replacement.
        3. No match — no parameter matches any canonical name.
           Parameters are assigned positionally: first → source_string,
           second → regex, third → replacement. Any beyond three get
           an empty string role.
        """
        canonical = ("source_string", "regex", "replacement")
        known = {p for p in string_params if p in canonical}

        if known == set(string_params):
            return {p: p for p in string_params}

        role_map: dict[str, str] = {}
        remaining_slots = [r for r in canonical if r not in known]
        slot_idx = 0

        for pname in string_params:
            if pname in known:
                role_map[pname] = pname
            else:
                if slot_idx < len(remaining_slots):
                    role_map[pname] = remaining_slots[slot_idx]
                    slot_idx += 1
                else:
                    role_map[pname] = ""
    ...
    def extract_parameters(self, prompt: str, fn: Any) -> dict[str, Any]:
        ...
        appearance in the prompt. String parameters are resolved to
        canonical roles (source_string, regex, replacement) before
        extraction — exact name matches use dedicated extractors,
        unrecognised names are filled positionally. Any remaining
        parameters fall through to logit-based ranking.
        """
        params: dict[str, Any] = {}
        ...
        role_map = self.resolve_string_roles(string_params_ordered)

        for pname, role in role_map.items():
            if role == "source_string":
                ...
                if has_in_empty:
                    ...
                    params[pname] = max(quoted, key=len)
                else:
                    params[pname] = ""

            elif role == "replacement":
                replacement_priority = self.extract_replacement_candidates(
                    prompt,
                )
